...
aePolygon::aePolygon()
{
	...
	hasMaterial = false;
	objRenderMode = POLYGON_NORMAL;
	objName = "";
	isTransparent = false;
	objRadius = 0.0;
}

aePolygon::aePolygon(char *objname)
{
	vertexList = new std::vector<aeVector3>();
	vertexList->resize(0);
	if(DEBUG) {
...
void aePolygon::Draw(aeEngineflags *eflags)
{
	aeVector3 tempvec;
	aeColor4 tempcolor;
	...
	glPushMatrix();
	  ...
	  if(objRenderMode == POLYGON_NORMAL)
		  glBegin(GL_POLYGON);
	  else
		  glBegin(GL_POLYGON);
