...
aeLine::aeLine()
{
	...
	hasMaterial = false;
	objRenderMode = LINES_NORMAL;
	objName = "";
	isTransparent = false;
	objRadius = 0.0;
}

aeLine::aeLine(char *objname)
{
	vertexList = new std::vector<aeVector3>();
	vertexList->resize(0);
	if(DEBUG) {
...
void aeLine::Draw(aeEngineflags *eflags)
{
	aeVector3 tempvec;
	aeColor4 tempcolor;
	...
	glPushMatrix();
	  ...
	  if(objRenderMode == LINES_NORMAL)
		  glBegin(GL_LINES);
	  else if(objRenderMode == LINES_STRIP)
		  glBegin(GL_LINE_STRIP);
	  else if(objRenderMode == LINES_LOOP)
