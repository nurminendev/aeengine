...
aeTriangle::aeTriangle()
{
	AECF_CNAME("aeTriangle")
...
aeTriangle::aeTriangle(char *objname)
{
	vertexList = new std::vector<aeVector3>();
	vertexList->resize(0);
	if(DEBUG) {
...
void aeTriangle::Draw(aeEngineflags *eflags)
{
	aeVector3 tempvec;
	aeColor4 tempcolor;
	...
	glPushMatrix();
	  ...
	  if(objRenderMode == TRIANGLES_NORMAL)
		  glBegin(GL_TRIANGLES);
	  else if(objRenderMode == TRIANGLES_STRIP)
		  glBegin(GL_TRIANGLE_STRIP);
	  else if(objRenderMode == TRIANGLES_FAN)
